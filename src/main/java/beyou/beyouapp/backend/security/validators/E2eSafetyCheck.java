 ...
 *
 * <p>Why: the e2e profile treats its database as throwaway — e2e runs hammer
 * it with destructive test data and assume a disposable schema. If the URL
 * accidentally points at the dev or production database, that contract is
 * violated. This component bails out BEFORE anything touches the datasource
 * so a misconfigured URL never gets a chance to pollute real data.
 *
    ...
    void verifyDatasourceIsThrowaway() {
        ...
        if (!isSafe) {
            throw new IllegalStateException(
                    ...
                    "The e2e profile assumes a disposable database and fills it with test data. " +
                    "Point at a database whose name contains 'e2e' or 'test' (e.g. beyou_e2e).");
        }

        log.info("[E2E SAFETY] Datasource '{}' looks safe for e2e (throwaway database).",
                datasourceUrl);
    }
}
