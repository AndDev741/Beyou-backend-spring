...
@Component
public class DesignDocsImportParser {

    public DesignTopicMetadata parseTopicYaml(String content, String fallbackKey) {
        ...
        DesignTopicStatus status = parseStatus(values.get("status"));

        return new DesignTopicMetadata(key.trim(), orderIndex, status);
    }
    ...
    private DesignTopicStatus parseStatus(String statusValue) {
        if (statusValue == null || statusValue.isBlank()) {
            return DesignTopicStatus.ACTIVE;
        }

        try {
            return DesignTopicStatus.valueOf(statusValue.trim().toUpperCase());
    ...
    public record DesignTopicMetadata(String key, int orderIndex, DesignTopicStatus status) {}
